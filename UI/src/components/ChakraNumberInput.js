...
function ChakraNumberInput({label, name, defaultValue, minValue, maxValue, required, allowMouseWheel, size, color, isDisabled, ...rest}) {
    const fontColor = useColorModeValue('gray', 'yellow.100');
    return (
        <Field name={name}>
            {({field, form}) => {
                return (
                    <FormControl isRequired={required} isInvalid={form.errors[name] && form.touched[name]}>
                        ...
                        <NumberInput color={fontColor} id={name}{...rest} size={size} allowMouseWheel={allowMouseWheel} defaultValue={defaultValue} min={minValue} max={maxValue}
